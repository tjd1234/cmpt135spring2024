...
// This is a "header file guard": it prevents the file from being included more
// than once.
#pragma once  
...
//    n > 0
// Post-condition:
//    Returns the nth number of the sequence defined as follows. The first
//    number is 3, and every number after that is twice the number before it
//    plus 1. The first few numbers of the sequence are 3, 7, 15, 31, and so on.
// Constraints:
...
//    Otherwise, returns false. In other words, returns true just when s is a
//    "ternary" string.
//
...
//    none
// Post-condition:
//    Returns true if s a valid string representation of an integer. An integer
//    a string of 1 or more digits that, optionally, starts with a single '-'.
//    No other characters are allowed.
//
//    For example, is_int returns true for all these strings: "911", "-23", "0",
//    "000", "-007", "19346".
//
//    And false for these ones: "", "-", "--", "0 ", " 911", "24e5", "782 2",
//    "+32", "3.14".
// Constraints:
//    Must be implemented using recursion (and no loops). You can write helper
//    functions if necessary.
// Note:
//    When testing a function that returns bool, make sure to test some cases
//    that make it return true, and some that make it return false.
//
...
//
//    For example, strip("  ab c  d ") returns "ab c  d". If s is the empty
//    string, or a string of all spaces, then "" is returned.
// Constraints:
//    Must be implemented using recursion (and no loops). You can write helper
//    functions if necessary.
//
