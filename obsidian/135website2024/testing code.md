...
Without seeing the body of the function, we can write some tests cases:
...
An advantage of this approach is that it can help you better understand how the function works, making it a little easier to implement. The test cases also give you examples that you can follow when implementing the function.
...
In general, [[blackbox testing]] tends to be preferable to [[whitebox testing]] because blackbox test cases will still work even if you change the implementation of the function. The [[whitebox testing]] cases might no longer be useful if the implementation changes.
...
In contrast, [[automated testing]] is when you write a test function to automatically call the function being tested and to automatically check that its output is correct. 
