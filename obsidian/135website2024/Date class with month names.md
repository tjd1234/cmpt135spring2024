...
//
// The name arrays are declared outside of the Date class to avoid making
// multiple copies of the arrays for each Date object.
//

const string short_months[] = {"Jan", "Feb", "Mar", "Apr", "May", "Jun",
                               "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"};

const string long_months[] = {"January", "February", "March", "April",
...
public:
    ...
    bool is_valid() const
    {
        return 1 <= day && day <= 31 && 1 <= month && month <= 12 && 0 <= year;
    }
    ...
    string get_long_month_name() const
    {
...
void test_names() {
    cout << "calling test_names() ... \n";
    Date d{1, 11, 2000}; // November 1, 2000
    assert(d.get_month() == 11);
    assert(d.get_short_month_name() == "Nov");
    assert(d.get_long_month_name() == "November");
    cout << "test_names() passed\n";
}
